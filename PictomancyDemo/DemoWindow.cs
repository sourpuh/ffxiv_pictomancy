...
public sealed class DemoWindow : Window, IDisposable
{
    ...
    public override void Draw()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Mirror")) Spawn(new MirrorObject { Position = pos, Rotation = rotation });
        ImGui.SameLine();
    ...
    // Unified fan/arc/cone/donut/circle. Inner=0 + 360 angle = circle; Inner>0 + 360 angle = donut;
    // Inner=0 + <360 angle = cone; Inner>0 + <360 angle = arc-band.
    private class FanObject : DemoObject
    {
        ...
        public int NumSegments = 0; // 0 = auto.
    ...
    // Vertical quad that mirrors the game's charaview texture (RenderTargetManager+0x2C8[0]).
    private class MirrorObject : DemoObject
    {
        public float Rotation;
        public float Width = 5f;
        public float HeightOffset = 2.5f;
        public Vector4 Tint = new(1f, 1f, 1f, 1f);
        public override string TypeName => "Mirror";
        public override void DrawUi()
        {
            ImGui.SliderFloat("Width (m)", ref Width, 0.5f, 30f);
            ImGui.SliderFloat("Height offset (m)", ref HeightOffset, 0f, 10f);
            ImGui.SliderAngle("Facing", ref Rotation);
            ImGui.ColorEdit4("Tint", ref Tint);
        }
        public override void DrawWorld(PctDrawList draw, PctDxParams baseParams)
        {
            var facing = new Vector3(MathF.Sin(Rotation), 0, MathF.Cos(Rotation));
            var center = Position + new Vector3(0, HeightOffset, 0);
            draw.AddMirror(center, facing, Width, ToU32(Tint), baseParams);
        }
    }

    // Screen-space rect that excludes pictomancy shapes; ignores world Position.
